        ...
        if (!opts.show) {
            ...
            that.bind(opts.event, function() {
                ...
                function resetInitTime() {
                    ...
                    var tempDefaultTime=that.val();
                    var tempDefaultTimes=tempDefaultTime.split(":");
                    var tempH=parseInt(tempDefaultTimes[0]);//%12;
                    var tempM=tempDefaultTimes[1];
                    initH=tempH;
                    initI=tempM;
...
//                createUL(); //动态生成控件显示的日期//需要
//                init_iScrll(); //初始化iscrll  年月日可以去掉
//                extendOptions(); //显示控件//需要
//                that.blur();
//                if (datetime) {
//                    showdatetime();
//                    refreshTime();
...
//                    $("#daywrapper ul li:eq(" + Math.round(indexD) + ")").html().substr(0, $("#daywrapper ul li:eq(" + Math.round(indexD) + ")").html().length - 1);
        ...
        function createULNew() {
            CreateDateUINew();
            //$("#yearwrapper ul").html(createYEAR_UL());
            //$("#monthwrapper ul").html(createMONTH_UL());
            //$("#daywrapper ul").html(createDAY_UL());
        }
        function CreateDateUINew() {
//            var str = '' +
...
//                '</div>';
            ...
            var str = '' +
                ...
                '<div id="datePage" class="page">' +
                '<section>' +
                '<div id="datetitle"><h1>时间设置</h1></div>' +
                '<div id="datemark"><a id="markyear"></a><a id="markmonth"></a><a id="markday"></a></div>' +
                '<div id="timemark"><a id="markhour"></a><a id="markminut"></a><a id="marksecond"></a></div>' +
